...
export default function HackingTheSystem() {
  ...
  const handleLogin = () => {
    ...
    if (isValidInjection) {
      ...
      if (username === "user@email.com" && password.trim() === "' or 1=1--") {
        setTimeout(() => {
          router.push("/lessons/sql-injection/hack-complete");
        }, 1000);
      }
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col justify-start">
      ...
      <main className="flex-grow flex flex-col items-center justify-start pt-6 px-6">
        <div className="max-w-6xl w-full flex flex-col lg:flex-row justify-center gap-10">
          ...
          <p>Password: <code className="text-gray-400 font-bold" style={{ letterSpacing: "-1.2px" }}>' or 1=1--</code></p>
            <Link href="/lessons/sql-injection/hack-complete">
          ...
          <div className="flex flex-row w-full max-w-6xl gap-6">
            <div className="w-1/2 bg-[#1e1e1e] text-green-200 rounded-md border border-gray-300 text-sm shadow-md">
              <div className="bg-gray-100 text-gray-600 px-3 py-1 font-semibold text-xs rounded-t-md">code</div>
              <pre className="font-mono text-sm whitespace-pre-wrap p-4">
              <span className="text-blue-400">SELECT</span> *<br></br>
                <span className="text-blue-400">   FROM</span> users<br></br>
                <span className="text-blue-400"> WHERE</span> email        =  <span className="text-yellow-400">'{username}'</span><br></br>
                <span className="text-blue-400">       AND</span> password =  <span className="text-yellow-400">' {password}'</span><br></br>
              </pre>
            </div>

            <div className="flex-1 shadow-lg border border-gray-300 rounded-lg overflow-hidden">
